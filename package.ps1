...
foreach($flavor in @('Portable','Lite')){
    ...
    & dotnet publish (Join-Path $PSScriptRoot 'desktop/BD2Sichuan.Desktop.csproj') -c Release --self-contained $selfContained "-p:SelfContained=$selfContained" "-p:PublishSelfContained=$selfContained" --artifacts-path $buildArtifacts --output $publish "-p:Version=$Version" "-p:EnableCompressionInSingleFile=$selfContained" -p:DebugType=None -p:DebugSymbols=false @restoreArgs --nologo
    if($LASTEXITCODE -ne 0){throw "Publish failed: $flavor"}
    $exe=Join-Path $publish 'BD2Sichuan.exe'
    ...
    if($identity.runtime -ne 'BD2Sichuan.Runtime2' -or $identity.compatibility -ne 'local-interface-adaptation' -or $identity.defaultIntervalMs -ne 1000){throw 'Embedded identity differs from release'}
    ...
    $configs=@(Get-ChildItem -LiteralPath (Join-Path $buildArtifacts 'bin') -Recurse -File -Filter 'BD2Sichuan.runtimeconfig.json')
    ...
    $name="BD2Sichuan-$Version-$flavor-win-x64"
